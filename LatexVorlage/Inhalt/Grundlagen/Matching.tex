...
Im Abschnitt  ~\ref{sec:m-based} wurden die feature-based Algorithmen erklärt, mit denen Deskriptoren für Merkmale in einem Bild generiert werden können.
...
\subsection{Vergleich von Deskriptoren mittels Distanzbestimmung}
\label{sec:DeskriptorenVergleichen}

Um zwei Deskriptoren zu vergleichen, wird die Distanz zwischen beiden berechnet. 
Die Distanz beschreibt wie ähnlich sich Deskriptoren sind. 
...
\label{sec:BFM}
Um Übereinstimmungen von Merkmalen in zwei Bildern zu finden, werden alle Deskriptoren des ersten Bildes mit allen Deskriptoren des zweiten Bildes verglichen. \cite{opencv}
...
Abhängig vom verwendeten Datensatz wählt die Bibliothek automatisch aus einer Reihe von Algorithmen, die für approximate nearest neighbor Suche optimiert sind, den besten aus. \cite{flann2009}
FLANN kann für den schnellen Vergleich von Deskriptoren eingesetzt werden.

Für größere Datensets arbeitet FLANN schneller als der Brute-Force Matcher. Allerdings kann durch die approximate nearest neighbor search nicht garantiert werden, dass FLANN die besten Matches findet. \cite{opencv}


\subsection{Filterung der Matching-Ergebnisse}
\label{sec:RatioTest}
Für die Deskriptoren aus dem ersten Bild wird jeweils der erst- und zweitbeste Match aus dem zweiten Bild gespeichert. Ein Match besteht dabei aus den beiden Deskriptoren, die verglichen werden, und die resultierende Distanz (Siehe Abschnit ~\ref{sec:DeskriptorenVergleichen}). Daraus ergibt sich eine 2xN Matrix, wobei N die Anzahl der Deskriptoren im ersten Bild darstellt.
...
Weil mit solchen Deskriptoren keine gute Aussage über die Ähnlichkeit von zwei Bildern getroffen werden kann, müssen diese herausgefiltert werden.
