...
func MatchRotationInvariantHashes(hash1 uint64, hashes2 []uint64, threshold int) (bool, uint64, time.Duration) {
	start := time.Now()
	for _, hash2 := range hashes2 {
		isMatch, _ := image_matching.HashesAreMatch(hash1, hash2, threshold, true)
		if isMatch {
			return true, hash2, time.Since(start)
		}
	}
